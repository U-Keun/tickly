<script lang="ts">
  ...
  async function loadItems() {
    try {
      items = await getItems(selectedCategoryId);
    } catch (error) {
  ...
  async function confirmReset() {
    try {
      await resetAllItems(selectedCategoryId);
  ...
  async function addItem(text: string, memo: string | null = null) {
    try {
      const newItem = await addTodoItem(text, selectedCategoryId);
      // If memo provided, save it immediately
      if (memo) {
        await updateItemMemo(newItem.id, memo);
        newItem.memo = memo;
      }
      items = [...items, newItem];
    } catch (error) {
  ...
  onMount(async () => {
    ...
    try {
      const wasReset = await checkAndAutoReset();
      if (wasReset) {
